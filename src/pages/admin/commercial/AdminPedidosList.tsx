...
export default function AdminPedidosList() {
  ...
  const fetchData = async () => {
    let q = supabase.from('pedidos').select('*, profiles(name)')

    if (debouncedSearch) {
      q = q.or(`numero_pedido.ilike.%${debouncedSearch}%,observacoes.ilike.%${debouncedSearch}%`)
    }
    if (status && status !== 'all') {
      q = q.eq('status', status)
    }
    if (dateRange?.from) {
      q = q.gte('data_pedido', dateRange.from.toISOString())
    }
    if (dateRange?.to) {
      q = q.lte('data_pedido', dateRange.to.toISOString())
    }
    if (sortConfig) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Pedidos / OS</CardTitle>
          <Button asChild>
            <Link to="new">Novo Pedido</Link>
          </Button>
        </CardHeader>
        <CardContent>
          <DataTableToolbar
