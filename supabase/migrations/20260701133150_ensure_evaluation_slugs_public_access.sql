-- ============================================================
-- Ensure all evaluation slugs are populated and public access works
-- Idempotent: safe to run multiple times
-- ============================================================

-- 1. Ensure evaluation_slug column exists
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS evaluation_slug TEXT;

-- 2. Populate evaluation_slug for all 10 standard services by title match
...
-- 3. Ensure all 10 services exist (insert if missing by slug)
DO $$
BEGIN
...
-- 4. Recreate unique partial index on evaluation_slug
DROP INDEX IF EXISTS idx_services_evaluation_slug_unique;
CREATE UNIQUE INDEX idx_services_evaluation_slug_unique
  ON public.services (evaluation_slug)
  WHERE evaluation_slug IS NOT NULL;

-- 5. RLS: Enable and allow public SELECT on services
...
-- 6. RLS: Allow anon INSERT into leads (public evaluation form)
DROP POLICY IF EXISTS "leads_anon_insert" ON public.leads;
CREATE POLICY "leads_anon_insert" ON public.leads
  FOR INSERT TO anon WITH CHECK (true);

-- 7. RLS: Allow anon INSERT/UPDATE on profiles (prospect registration)
...
-- 8. Ensure functions exist and are callable by anon
...
-- 9. Ensure leads.service_id column exists with FK
