DO $$
BEGIN
  -- Cleanup duplicates for stripe_config to support unique constraint
  DELETE FROM public.stripe_config
  WHERE id NOT IN (
    SELECT id
    FROM (
      SELECT id, ROW_NUMBER() OVER(PARTITION BY tenant_id ORDER BY id) as rn
      FROM public.stripe_config
    ) t
    WHERE t.rn = 1
  );
  ...
  -- Cleanup duplicates for asaas_config to support unique constraint
  DELETE FROM public.asaas_config
  WHERE id NOT IN (
    SELECT id
    FROM (
      SELECT id, ROW_NUMBER() OVER(PARTITION BY tenant_id ORDER BY id) as rn
