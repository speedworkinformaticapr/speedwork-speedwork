...
BEGIN
  ...
  SELECT (pos - 1) INTO v_config_index
  FROM jsonb_array_elements(v_menu_config) WITH ORDINALITY arr(elem, pos)
  WHERE elem->>'id' = 'configuracoes';

  IF v_config_index IS NULL THEN
    RETURN;
  END IF;

  v_has_submenus_key := v_menu_config->v_config_index ? 'submenus';

  IF v_has_submenus_key THEN
    v_submenus := v_menu_config->v_config_index->'submenus';
  ELSE
    v_submenus := v_menu_config->v_config_index->'items';
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE item->>'id' = 'whatsapp-config'
       OR item->>'url' = '/admin/whatsapp'
  ) INTO v_item_exists;
  ...
  FOR v_idx IN 0..jsonb_array_length(v_submenus) - 1 LOOP
    ...
    IF v_item->>'id' = 'email-config' THEN
  ...
  v_submenus := jsonb_insert(
    ...
    '{"id": "whatsapp-config", "label": "Gestão WhatsApp", "url": "/admin/whatsapp"}'::jsonb
  );

  IF v_has_submenus_key THEN
    v_menu_config := jsonb_set(
      v_menu_config,
      ARRAY[v_config_index::text, 'submenus'],
      v_submenus
    );
  ELSE
    v_menu_config := jsonb_set(
      v_menu_config,
      ARRAY[v_config_index::text, 'items'],
      v_submenus
    );
  END IF;
